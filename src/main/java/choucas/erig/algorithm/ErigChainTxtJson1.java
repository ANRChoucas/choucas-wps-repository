...
@Algorithm(version = "1.0.0", title = "Erig Chain Txt to Json process", abstrakt = "Extraction of spatial features and spatial relationships from text and toponyms location "
		...
		+ "Documentation : see  http://erig.univ-pau.fr/PERDIDO/api.jsp for documentation on compound services : POS processing, Expanded Named Entity Recognition, GetToponyms and GeoJson. ")

public class ErigChainTxtJson1 extends AbstractAnnotatedAlgorithm {

	// private static final Logger log =
	// LoggerFactory.getLogger(ErigChainTxtJson1.class);
	...
	public void run() {
		...
		if (textInput != null) {
			...
			try {
				...
				output = callNER(url_base, api_key, lang, input);
				if (WsUtils.getStdoutFlag()) {
					System.out.println("\nInput :\n" + input);
					System.out.println("\nOutput :\n" + output);
				}
